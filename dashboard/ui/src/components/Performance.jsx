...
export default function Performance() {
  ...
  if (!p.total_trades) return (
    <div className="empty"><div className="empty-icon">📊</div><div className="empty-text">No closed trades yet. Close positions to see performance analytics.</div></div>
  );
      ...
      <div className="stats-grid">
        ...
        {p.best_trade && (
          <div className="stat stat-green">
            ...
            <div className="stat-value pos">+{p.best_trade.pnl_percent?.toFixed(2)}%</div>
            <div className="stat-sub">{p.best_trade.symbol}</div>
          </div>
        )}
        {p.worst_trade && (
          <div className="stat stat-red">
            <div className="stat-label">Worst Trade</div>
            <div className="stat-value neg">{p.worst_trade.pnl_percent?.toFixed(2)}%</div>
            <div className="stat-sub">{p.worst_trade.symbol}</div>
          </div>
        )}
      </div>
